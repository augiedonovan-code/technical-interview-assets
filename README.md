# Data Visualization Case Study — Netflix Content Catalog

## Objective
This case study evaluates your ability to:
- Explore an unfamiliar dataset independently
- Identify meaningful patterns and trends
- Create clear, aesthetically pleasing visualizations
- Communicate insights to stakeholders

---

## Dataset Overview
You will use the Netflix Movies and TV Shows dataset provided in this repository.

Each row represents one Netflix title and includes
- Content type (Movie or TV Show)
- Release year
- Date added to Netflix
- Country of production
- Genre classifications
- Content rating
- Duration


---

## Tools
- **Tableau Public** (required)  
  https://www.tableau.com/products/public/download

You may use other tools (SQL, Python, Excel) for data preparation, but all final visualizations must be created in Tableau Public.

---

## Case Study Prompt
You are an analyst asked to help leadership better understand this dataset

Using the dataset:

1. Create a Tableau dashboard that highlights important trends and patterns.
2. Identify 3–5 key insights that would be meaningful to senior executives.
3. Provide diagnostic commentary explaining *why* these patterns may exist.
4. Based on your findings, propose 1–2 strategic recommendations related to:
   - Content mix and balance
   - Genre or format concentration
   - Geographic representation
   - Areas of potential over- or under-investment

There is no single correct answer. Focus on clarity, reasoning, and communication.

---

## Deliverables

### 1. Tableau Public Workbook 
- Clear visual hierarchy and layout
- Appropriate chart selection
- Thoughtful use of color, labels, and annotations
- Primarily static design; advanced interactivity (action filters, parameters ect) is not required and should be used only if necessary to support presentation flow

### 2. Presenation (5-10 minutes)
Include:
- 3–5 executive-level insights
- Brief diagnostic explanation for each insight
- 1–2 strategic recommendations
- Key assumptions and data limitations

---


## Evaluation Criteria
You will be evaluated on:
- Analytical reasoning and curiosity
- Visual clarity and design quality
- Strength and relevance of insights
- Executive-level communication
- Quality of recommendations
- Transparency around assumptions and limitations




- Perfection is not expected — thoughtfulness and clarity are. And have fun with it--data is fun

